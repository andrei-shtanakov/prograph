...
async def test_mcp_describe_project(indexed_mcp_fixture: Path):
    async with stdio_client(_server_params(indexed_mcp_fixture)) as (read, write):
        async with ClientSession(read, write) as session:
            await session.initialize()
            result = await session.call_tool("describe_project", arguments={"name": "py_client"})
            payload = json.loads(_text(result.content))
...
async def test_mcp_edge_evidence_for_mcp_call(indexed_mcp_fixture: Path):
    async with stdio_client(_server_params(indexed_mcp_fixture)) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            edges = json.loads(
                _text(
                    (await session.call_tool("find_edges", arguments={"kind": "mcp_call"})).content
                )
            )
            mcp_edge_id = edges[0]["id"]
            result = await session.call_tool("edge_evidence", arguments={"edge_id": mcp_edge_id})
